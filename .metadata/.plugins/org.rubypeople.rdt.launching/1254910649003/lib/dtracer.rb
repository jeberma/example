...
--------------------------------------------------------- Class: DTracer
     ...
     #ifdef ENABLE_DTRACE #include "dtrace.h" #endif

     VALUE rb_mDtrace;

     static VALUE ruby_dtrace_probe(int argc, VALUE *argv, unsigned
     check_only) { #ifdef ENABLE_DTRACE
     ...
     static VALUE ruby_dtrace_fire(int argc, VALUE *argv, VALUE klass) {

       return ruby_dtrace_probe(argc, argv, 0);

     }

     static VALUE ruby_dtrace_enabled(VALUE klass) {
